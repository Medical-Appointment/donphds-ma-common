...
public class KeyProvider {
  ...
  private final AsyncHttpClient client = Dsl.asyncHttpClient(new DefaultAsyncHttpClientConfig.Builder()
  ...
  private CompletableFuture<List<SecretKey>> generateKey() {
    return CompletableFuture.<List<SecretKey>>supplyAsync(() ->
      Collections.singletonList(new SecretKey("map-db-key", "jWJTpS9z/2CZSXMxUb9dP90ISZsys09M", 1)));
  }

  private CompletableFuture<List<SecretKey>> fetch() {
    return client.prepareGet(endpoint).execute().toCompletableFuture().thenApply(this::parseResponse);
  }
  ...
  private List<SecretKey> parseResponse(Response rsp) {
    ...
    if (HttpStatus.valueOf(statusCode).is4xxClientError()) {
      throw new NotFoundException("not db aes key fetch");
    }
    ObjectMapper om = new ObjectMapper();
    try {
      return om.readValue(res.body(), new TypeReference<List<SecretKey>>() {
      });
