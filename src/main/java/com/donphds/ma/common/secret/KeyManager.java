package com.donphds.ma.common.secret;


import com.donphds.ma.common.exception.NotFoundException;
import com.donphds.ma.common.pojo.vo.SecretKey;
import com.donphds.ma.common.utils.CryptUtil;
...
public class KeyManager implements ApplicationListener<ContextRefreshedEvent> {
  ...
  static {
    DEFAULT_KET = new SecretKey("default key", Base64.getEncoder().encodeToString(KeyProvider.DEFAULT_AES_KEY), 0);
  }
  ...
  public KeyManager(KeyProvider provider) {
    cache = Caffeine.newBuilder().maximumSize(1000L).
      expireAfterWrite(65, TimeUnit.MINUTES).refreshAfterWrite(30, TimeUnit.MINUTES)
      .executor(new ThreadPoolExecutor(3, 4, 1,
  ...
  public SecretKey getKey(int version) {
    ...
    if (Objects.isNull(secretKey)) {
      throw new NotFoundException(String.format("current version %s has not secret key", version));
    }
    return secretKey;
  }

  public void load() {
    this.cache.get(CACHE_SECRET_KET).whenComplete(((keys, e) -> {
      if (e == null) {
