...
import com.donphds.ma.common.exception.CryptoException;
...
public class CryptUtil {
  ...
  public static String decryptToString(String cipher) {
    return StringUtils.isBlank(cipher) ? cipher
  ...
  private static byte[] disassembly(byte[] cipher) {
    ...
    return process(ciphertext, Base64.getDecoder().decode(key.getAseKey()), iv, Cipher.DECRYPT_MODE);
  }

  private  static byte[] encrypt(byte[] plaintext) {
    SecretKey key = keyManager.getLatestKey();
    byte[] iv = generateIV();
    byte[] cipherText = process(plaintext, Base64.getDecoder().decode(key.getAseKey()), iv, Cipher.ENCRYPT_MODE);
